...
def tournament(T,M1,M2,rounds,m,SS,type_of_graph,draw_graph):     
    R = [0 for i in range(len(T))]
...
def next_round(T, R, deletions,SS):
    if 2*deletions > len(T):
        print 'Deletes and additions overlap! Use more agents.'
    ...
    # Add in 'deletion' lots of new mix players:
    T = np.concatenate([T,[mix_agent for i in range(deletions)]])
    ...
    remaining_mix_states = [i for i in SS if i!=0]+[Mix for i in range(deletions)]
    for i in range(len(T)):
        if T[i]==mix_agent:
            SS[i] = remaining_mix_states.pop()
    
    return T, SS

# Put all the definitions together into one function.
def multiple_tournament(P,M1,M2,rounds,deletions,total_tournaments,m,type_of_graph,draw_graph):
    ALL_T = []
    T,R = tournament_first_round(P,M1,M2,rounds,m,type_of_graph,draw_graph)
    SS = [0 for i in range(len(T))]
    
    ALL_T.append(copy.copy(T))
    T, M = next_round(T, R, deletions,SS) # Next round array of players and mix array
    
    for i in range(total_tournaments-1):
        T,R = tournament(T,M1,M2,rounds,m,SS,type_of_graph,draw_graph)
        ALL_T.append(copy.copy(T))
        T,SS = next_round(T, R, deletions,SS) # Next round array of players and mix array
